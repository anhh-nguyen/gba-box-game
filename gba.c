...
void drawFullScreenImageDMA(const u16 *image) {
  ...
  DMA[3].cnt = (WIDTH * HEIGHT) | DMA_16 | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
}

void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for (int i = 0; i < height; i++) {
    DMA[3].src = &image[OFFSET(i, 0, width)]; //copy from the imagine one by one line
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for (int i = 0; i < height; i++) {
    DMA[3].src = &image[OFFSET(row + i, col, WIDTH)]; //copy from the imagine one by one line
    DMA[3].dst = &videoBuffer[OFFSET(row + i, col, WIDTH)];
    DMA[3].cnt = width | DMA_SOURCE_INCREMENT | DMA_ON;
  }
}
